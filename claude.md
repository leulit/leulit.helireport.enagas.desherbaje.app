...
- **`Get.offAllNamed` a una ruta que YA está en la pila mata el controller recién creado, no el viejo.** Flutter no libera una ruta marcada para borrado mientras la de arriba está animando (`navigator.dart`: `_RouteLifecycle.removing` + `canRemoveOrAdd`), así que el binding de la ruta nueva corre ANTES de que muera la vieja — y el borrado por ruta de GetX es por TIPO (`_removeDependencyByRoute` → `delete(key: 'XController')`). Resultado: al terminar la transición, la ruta vieja se lleva por delante la instancia NUEVA. Síntomas que no se parecen entre sí y son el mismo bug: `A TextEditingController was used after being disposed` al tocar el buscador de la lista (el `onClose` del controller vivo ya corrió), workers/debounce muertos, y una instancia zombi que queda registrada (`delete` sale por la rama `lateRemove` y NO borra la clave) y se reutiliza en el siguiente acceso — de ahí que un desplegable muestre los datos de la visita anterior: `myOnInit` nunca vuelve a leer `Get.arguments`. GetX tiene el mecanismo para absorberlo (`lateRemove`: con el factory `isDirty`, `delete` mata la instancia ANTERIOR y respeta la nueva), pero `_insert` crea el factory de reemplazo con `isDirty = false` (get 4.7.3), así que nunca se arma. Regla: los controllers de pantalla se registran en el binding con `putScreenController<T>` (`lib/core/screen_controller.dart`), que hace `lazyPut` + `GetInstance().markAsDirty<T>()` — nunca `Get.lazyPut` a pelo. Efecto colateral deseado: cada entrada a la pantalla crea instancia nueva. Cubierto por `test/core/screen_controller_test.dart` (incluye el test que reproduce el bug sin el helper). (Fix 2026-08-18.)
