...
- `POST /positions/batch` idempotente por `traza_client_id` (payload: `FeatureCollection`/`MultiLineString`, no batches de puntos sueltos) para subida de trazas GPS. **Pendiente:** el cuerpo puede superar los 4 MB en trazas largas (jornada completa a 1 muestra/5m) — confirmar/subir el límite de tamaño de body aceptado por el endpoint.
